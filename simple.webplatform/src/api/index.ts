...
// Check adb binary existence with automatic fallback to common Android SDK / platform-tools locations.
async function _detectAdb(): Promise<boolean> {
	if (await isCommandAvailable("adb")) {
		return true;
	}

	// Probe common Windows/macOS/Linux platform-tools paths
	const userProfile = process.env.USERPROFILE || process.env.HOME || "";
	const localAppData = process.env.LOCALAPPDATA || "";
	const candidateDirs = [
		path.join(userProfile, "platform-tools"),
		path.join(localAppData, "Android", "Sdk", "platform-tools"),
		"C:\\platform-tools",
		"C:\\Users\\U-ser\\platform-tools"
	];

	const fs = await import("node:fs");
	for (const candidate of candidateDirs) {
		const adbExecutable = process.platform === "win32" ? path.join(candidate, "adb.exe") : path.join(candidate, "adb");
		try {
